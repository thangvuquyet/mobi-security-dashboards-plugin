...
import defaultBrandImage from '../../assets/ui/mobisecurity_logo.svg';
...
export function LoginPage(props: LoginPageDeps) {
  ...
  return (
    <>
      <EuiListGroup className="login-wrapper mobisecurityLoginCard">
        {props.config.ui.basicauth.login.showbrandimage && (
          <EuiImage
            className="mobisecurityLoginCard__logo"
            size="fullWidth"
            alt="MobiSecurity"
            url={props.config.ui.basicauth.login.brandimage || defaultBrandImage}
          />
        )}
        <EuiText className="mobisecurityLoginCard__title" textAlign="center">
          <div className="mobisecurityLoginHeading">
            <h1 className="mobisecurityLoginProduct">
              Nền tảng giám sát và phân tích an toàn thông tin
            </h1>
          </div>
        </EuiText>
        <EuiForm className="mobisecurityLoginCard__form" component="form">
          {formOptions(props.config.auth.type)}
          {errorLabel}
        </EuiForm>
      </EuiListGroup>
      <footer className="mobisecurityLoginFooter">
        Sản phẩm được phát triển bởi <strong>Trung tâm CNC MobiFone</strong>
      </footer>
    </>
  );
}
